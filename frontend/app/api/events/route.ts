...
export async function GET(request: NextRequest) {
    try {
        ...
        // Only EVENT_MANAGER can list events
        if (authUser.role !== "EVENT_MANAGER") {
            return NextResponse.json(
                { error: "Only event managers can list events" },
                { status: 403 }
            );
        }
        ...
        const [events] = (await db.execute(
            "SELECT id, name, description, start_date, end_date, owner_id, created_at, updated_at FROM events ORDER BY created_at DESC"
        )) as [any[], any];
